...
function getEvent (string, context) {
  if (typeof string === 'string') {
    const matches = names.filter(n => n.name === string.toLowerCase())
    if (matches.length > 0) {
      const obj = matches[0]
      if (context === 'config' && getInt(obj.name)) { return obj.name }
...
module.exports = {
  ...
  getEnabled (disabled) {
    ...
    for (let i = 0; i < names.length; i++) {
      event = names[i]
      if (event.level === 0 && (!disabled || !disabled.includes(event.name))) { list.push(event.name) }
    }
    return list
  },
  ...
  /**
     ...
     */
  getType (event) {
    if (events[getEvent(event)]) { return events[getEvent(event)].type }
    throw new Error(`Unrecognized event: ${event}`)
  },
  ...
  /**
     ...
     */
  getName (event) {
    if (events[getEvent(event)]) { return events[getEvent(event)].name }
    throw new Error(`Unrecognized event: ${event}`)
  },
  ...
  countScrambles (event) {
    if (events[getEvent(event)]) { return events[getEvent(event)].scrambles }
    return undefined
  },
