...
module.exports.run = async (bot, message, args, cube) => {
	...
	if(args[0] == "reset") {
		...
		if(guild.restricted.includes(channelID)) {
			if(!guild.restricted[1]) await bot.guildData.updateOne({ guildID: message.guild.id }, { $unset: { restricted: "" } });
			...
			await bot.guildData.updateOne({ guildID: message.guild.id }, { $push: { ignore: channelID } }, { upsert: true });
